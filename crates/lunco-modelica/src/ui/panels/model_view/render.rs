...
fn render_unified_toolbar(
    ...
    ui.horizontal(|ui| {
        ...
        if let Some(ref err) = compilation_error {
            if ui.colored_label(egui::Color32::LIGHT_RED, "⚠").on_hover_text(err).clicked() { dismiss_error = true; }
        } else {
            match compile_state {
        ...
        ui.separator();
        let runner_busy = world.get_resource::<crate::ModelicaRunnerResource>().map(|r| r.0.is_busy()).unwrap_or(false);
        ...
        if let Some(model_ref) = model_ref {
            ...
            let bounds = drafted_bounds.unwrap_or_else(|| {
                world.get_resource::<crate::ModelicaRunnerResource>().and_then(|r| {
                    ...
                    }).unwrap_or(lunco_experiments::RunBounds { t_start: 0.0, t_end: 1.0, dt: None, tolerance: None, solver: None })
            });
