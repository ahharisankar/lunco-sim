...
use crate::document::duplicate::{
    collect_parent_imports, extract_class_spans_inline, extract_class_spans_via_path,
    find_class_byte_range, rewrite_inject_in_one_pass,
};
...
pub struct NewPlotPanel {
    ...
    /// Source `VizId.0` to duplicate from. `0` (default) ⇒ open a
    /// blank plot. Non-zero ⇒ clone the source plot's signal bindings
    /// AND its per-panel picked-vars / scrub state into the new tab.
    /// `signals` is appended on top of the cloned set.
    pub source: u64,
}
...
fn on_new_plot_panel(trigger: On<NewPlotPanel>, mut commands: Commands) {
    ...
    commands.queue(move |world: &mut World| {
        ...
        // Clone source plot's bindings (live cosim) and picked_vars
        // (experiments overlay) when duplicating. Reads happen up
        // front so we don't double-borrow the registry below.
        ...
        let title = if !ev.title.is_empty() {
            ...
            // Suffix the source's title with "(copy)" for clarity in
            // the dock row. Falls back to "Plot #N" if source has no
            // title.
            source_viz
                .and_then(|src| {
                    world
        ...
        for s in &ev.signals {
            ...
            // Skip if the source already had this exact path bound.
